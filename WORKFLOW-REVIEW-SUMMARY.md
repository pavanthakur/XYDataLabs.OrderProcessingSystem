...
2. **Optional**: The git configuration hint can be safely ignored as it is purely informational and generated by the checkout action itself. It does not affect workflow execution.

3. **Consider enabling Pre-Deployment Validation** once Azure OIDC setup is complete (currently disabled in the `infra-deploy.yml` workflow with `if: false` condition in the `pre-validate` job)
