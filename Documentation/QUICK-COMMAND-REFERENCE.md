...
|---------|-------------|--------------|
| `/xylab-day-complete` | End of every curriculum day | Routes updates to curriculum, commands files, ADRs, memory. Suggests commit. |
| `/xylab-sql-local-access` | After every fresh bootstrap/deploy | Opens/closes Azure SQL firewall for your local IP. Prints SSMS details. |

**How to run:** `Ctrl+Shift+I` → Agent mode → `/xylab-day-complete` → answer "What did you complete today?"
...
**How to run:** `Ctrl+Shift+I` → Agent mode → type `/xylab-day-complete` → answer "What did you complete today?"
