...
- When containers are Healthy/Running, the script exits 0 even if Docker Compose prints status lines on stderr.
- See the comprehensive guide for details: `Documentation/02-Docker-Guides/DOCKER_COMPREHENSIVE_GUIDE.md` (Strict Mode section).

### Compose Auto-Detection (Nov 16 2025)
The startup script now detects and prefers the v2 plugin form `docker compose` automatically. If only the legacy binary exists, it falls back to `docker-compose`. If neither is found it exits with an error instructing you to install Docker Desktop. All documented commands remain valid; you may freely substitute `docker compose` anywhere `docker-compose` appears.
