...
if ((Test-Path $envLocal) -and -not $Force) {
    ...
    Write-Host '  Enter passwords for your local sandbox (never used in Azure):' -ForegroundColor Yellow
    $sqlPassword  = Read-Password '  SQL Server password  (LOCAL_SQL_PASSWORD) '
    $certPassword = Read-Password '  HTTPS cert password  (LOCAL_CERT_PASSWORD)'
