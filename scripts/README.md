...
- If legacy local values or dev Key Vault secret names are swapped (`MerchantId`/`PrivateKey`), `setup-local.ps1` normalizes them before writing `.env.local` and API user-secrets.
...
- Accepts either a GitHub Actions run URL, job URL, or explicit `RunId` / `JobId`
...
**Usage**:
```powershell
pwsh -NoProfile -ExecutionPolicy Bypass -File .\scripts\export-github-run-log.ps1 -Url "https://github.com/pavanthakur/XYDataLabs.OrderProcessingSystem/actions/runs/29636818068"
...
**Output**:
- `TestResults/GitHubActions/<run-id>/run-<run-id>.log`
- `TestResults/GitHubActions/<run-id>/run-<run-id>.summary.txt`
...
**Diagnosis Flow**:
1. Run the script from a normal PowerShell terminal with the failed GitHub Actions run or job URL.
