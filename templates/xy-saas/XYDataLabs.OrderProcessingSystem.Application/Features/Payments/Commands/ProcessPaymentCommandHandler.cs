...
public sealed class ProcessPaymentCommandHandler : ICommandHandler<ProcessPaymentCommand, Result<PaymentDto>>
{
        ...
        ITenantProvider tenantProvider)
    {
        ...
        ArgumentNullException.ThrowIfNull(paymentGatewayOptions);
        ...
        _redirectUrl = _paymentGatewayOptions.RedirectUrl;
        _context = context;
        _timeProvider = timeProvider;

        _tenantProvider = tenantProvider;
        _paymentProvider = appMasterData.GetProviderByNameForTenant(_paymentGatewayOptions.ProviderName, tenantProvider.TenantId)
            ?? throw new InvalidOperationException($"Payment provider '{_paymentGatewayOptions.ProviderName}' was not found in master data for tenant {tenantProvider.TenantId}");
    }
    ...
    public async Task<Result<PaymentDto>> HandleAsync(ProcessPaymentCommand command, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var resolvedDeviceSessionId = string.IsNullOrWhiteSpace(command.DeviceSessionId)
                ? _paymentGatewayOptions.DeviceSessionId
            ...
            var (gatewayCustomer, billingCustomerId) = await CreateCustomerAsync(request, paymentMethod, cancellationToken);
            await UpdatePaymentMethodByBillingCustomerIdAsync(paymentMethod.Id, billingCustomerId, cancellationToken);
            var createdCard = await CreateCardTokenAsync(request, gatewayCustomer, billingCustomerId, customerOrderId, attemptOrderId, paymentTraceId, cancellationToken);
            ...
            var charge = await CreateChargeAsync(request, gatewayCustomer, createdCard.Id, paymentMethod, billingCustomerId, customerOrderId, attemptOrderId, paymentTraceId, isThreeDSecureEnabled, redirectUrl, cancellationToken);
            var normalizedChargeStatus = EnumHelper.NormalizePaymentProviderStatus(charge.Status)
    ...
    private async Task<Domain.Entities.PaymentMethod> CreatePaymentMethodAsync(CancellationToken cancellationToken)
    {
        ...
        var providerIdInTenantDb = await _context.PaymentProviders
            .Where(p => p.Name == _paymentProvider.Name)
            .Select(p => p.Id)
    ...
    private async Task<(PaymentGatewayCustomer gatewayCustomer, int BillingCustomerId)> CreateCustomerAsync(
        CustomerWithCardPaymentRequestDto request,
        Domain.Entities.PaymentMethod paymentMethod,
        CancellationToken cancellationToken)
    {
        _logger.LogInformation("Creating customer in payment gateway {ProviderName}...", _paymentProvider.Name);
        ...
        var gatewayCustomer = await _paymentGatewayService.CreateCustomerAsync(new PaymentGatewayCustomer
        {
            Name = request.Name,
            Email = request.Email,
            RequiresAccount = false
        ...
        _logger.LogInformation("Customer created with ID: {CustomerId}", gatewayCustomer.Id);

        var billingCustomer = request.ToBillingCustomer();
        billingCustomer.APICustomerId = gatewayCustomer.Id;
        ...
        return (gatewayCustomer, billingCustomer.Id);
    }

    private async Task<PaymentGatewayCardToken> CreateCardTokenAsync(
        CustomerWithCardPaymentRequestDto request,
        PaymentGatewayCustomer gatewayCustomer,
        ...
        CancellationToken cancellationToken)
    {
        _logger.LogInformation("Creating card token in payment gateway {ProviderName}...", _paymentProvider.Name);

        var card = new PaymentGatewayCardTokenRequest
        {
        ...
        var cardTransaction = new CardTransaction
        {
            ...
            CustomerOrderId = customerOrderId,
            AttemptOrderId = attemptOrderId,
            TransactionStatus = EnumHelper.GetEnumDescription(PaymentProviderTransactionStatus.Completed),
        ...
        var statusHistory = new TransactionStatusHistory
        {
            ...
            Status = EnumHelper.GetEnumDescription(PaymentProviderTransactionStatus.Completed),
            AttemptOrderId = attemptOrderId,
    ...
    private async Task<PaymentGatewayCharge> CreateChargeAsync(
        CustomerWithCardPaymentRequestDto request,
    ...
    {
        _logger.LogInformation(
            "Creating charge in payment gateway {ProviderName} for payment trace {PaymentTraceId}, customer order {CustomerOrderId}, attempt order {AttemptOrderId}, 3DS enabled {IsThreeDSecureEnabled}",
        ...
        var chargeRequest = new PaymentGatewayChargeRequest
        {
            ...
            UseThreeDSecure = isThreeDSecureEnabled,
            RedirectUrl = redirectUrl,
            Customer = customer
        };

        var charge = await _paymentGatewayService.CreateChargeAsync(chargeRequest);
        ...
        var payinLogDetails = new PayinLogDetails
        {
            ...
            AdditionalInfo = BuildTrackingInfo(paymentTraceId, customerOrderId, chargeRequest.OrderId, threeDSecureStage, "Charge created in payment gateway"),
            PaymentTraceId = paymentTraceId,
            ThreeDSecureStage = threeDSecureStage,
            CreatedBy = billingCustomerId,
            CreatedDate = _timeProvider.GetUtcNow().UtcDateTime
