...
static async Task<SmokeCheck> PublishAsync(
...
static async Task<SmokeCheck> ReceiveExpectedAsync(
    ServiceBusClient client,
    string topicName,
...
static async Task<SmokeCheck> DeadLetterExpectedAsync(
    ServiceBusClient client,
    string topicName,
    string subscriptionName,
    string expectedMessageId,
...
static string Escape(string value) => value.Replace("|", "\\|", StringComparison.Ordinal).ReplaceLineEndings(" ");
