...
namespace XYDataLabs.OrderProcessingSystem.Orders.Host;
...
internal sealed class KeycloakIntrospectionAuthenticationHandler : AuthenticationHandler<AuthenticationSchemeOptions>
{
    ...
    private async Task<AuthenticateResult> ValidateTokenAsync(string authority, string clientId, string clientSecret, string token)
    {
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(expectedAudience) && !ContainsAudience(payload.Audience, expectedAudience))
            {
    ...
    private sealed record KeycloakIntrospectionResponse(
        ...
        [property: System.Text.Json.Serialization.JsonPropertyName("tenant_code")] string? TenantCode,
        [property: System.Text.Json.Serialization.JsonPropertyName("aud")] JsonElement Audience,
        [property: System.Text.Json.Serialization.JsonPropertyName("realm_access")] KeycloakRealmAccess? RealmAccess);

    private sealed record KeycloakRealmAccess(
        [property: System.Text.Json.Serialization.JsonPropertyName("roles")] IReadOnlyCollection<string>? Roles);
}
