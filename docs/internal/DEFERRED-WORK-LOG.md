...
| DW-010 | Implement Central Tenant Registry: extract tenant identity and per-tenant provider assignments from `DbInitializer` code into an ops-owned Registry DB accessible only by App Managed Identity | Phase 8.5 closeout / security posture | Architecture owner | `DbInitializer.SeedProviderAssignments` is a safe placeholder at this stage — explicit, PR-reviewed, version-tracked. Moving to the Registry before Phase 8.6 scope is defined would be speculative. The hard blocker is that real tenant identities and provider assignments must never be visible to developers in non-local environments. | Developer visibility of real tenant-to-provider business data; change path for provider reassignment requires a code deploy instead of an ops action | 2026-06-30 | Phase 8.6 begins; `ITenantRegistry` interface is defined in Application layer and `SeedProviderAssignments` is removed. Closure trigger: staging Registry DB access-controlled and ADR-019 merged. | Open |
