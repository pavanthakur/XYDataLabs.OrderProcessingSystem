...
try {
    ...
    if ($kvs -and $kvs.Count -gt 0) {
        $kvs | ForEach-Object { 
            Write-Host "  ✅ Key Vault: Write-Host "╔════════════════════════════════════════════════════════════════╗" -ForegroundColor Cyan
Write-Host "║        AZURE LEARNING PROGRESS VERIFICATION - DEV              ║" -ForegroundColor Cyan
Write-Host "╚════════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
...
" -ForegroundColor Green
            try {
                $secrets = az keyvault secret list --vault-name Write-Host "╔════════════════════════════════════════════════════════════════╗" -ForegroundColor Cyan
Write-Host "║        AZURE LEARNING PROGRESS VERIFICATION - DEV              ║" -ForegroundColor Cyan
Write-Host "╚════════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
...
$global:VerificationResults = @{
 ...
 --query "[].name" -o json 2>$null | ConvertFrom-Json
                if ($secrets) {
                    $secretCount = if ($secrets -is [array]) { $secrets.Count } else { 1 }
                    Write-Host "    ├─ Secrets stored: $secretCount" -ForegroundColor Green
                    
                    # List secret names
                    if ($secrets -is [array]) {
                        $secrets | ForEach-Object { Write-Host "    ├─ • Write-Host "╔════════════════════════════════════════════════════════════════╗" -ForegroundColor Cyan
Write-Host "║        AZURE LEARNING PROGRESS VERIFICATION - DEV              ║" -ForegroundColor Cyan
Write-Host "╚════════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
...
" -ForegroundColor Gray }
                    ...
                    Write-Host "    └─ Secrets: None stored (empty)" -ForegroundColor Yellow
                }
            } catch {
                Write-Host "    └─ Secrets: Unable to list (check permissions)" -ForegroundColor Yellow
            }
        }
    } else {
        Write-Host "  ⚠️  No Key Vaults found - Will add to infrastructure" -ForegroundColor Yellow
