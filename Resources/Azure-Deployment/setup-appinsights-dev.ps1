param(
  ...
  [Parameter(Mandatory=$false)][string]$UiAppInsights  = "ai-orderprocessing-ui-dev"
)
...
Write-Info "Setting APPLICATIONINSIGHTS_CONNECTION_STRING and enabling auto-instrumentation on the API web app..."
...
Write-Info "Skipping direct UI App Insights setup for '$UiAppName' because the React SPA forwards browser payment telemetry through /payment/client-event on the API."
...
Write-Info "UI App Service diagnostics still flow to Log Analytics, but direct UI App Insights wiring is intentionally skipped for the React SPA host."
