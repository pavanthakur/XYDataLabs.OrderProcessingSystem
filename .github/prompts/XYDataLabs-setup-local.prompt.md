---
agent: agent
description: "After a fresh git clone: runs setup-local.ps1 to create .env.local, set dotnet user-secrets, and trust the HTTPS dev cert; then summarises VS F5 ports (API :5010/:5011, UI :5173/:5174), Docker launch commands, and how OpenPay sandbox credentials are sourced"
---
...
- **VS F5**: select `http` or `https` profile (not `docker-*` profiles) → HTTP: API at http://localhost:5010/swagger, UI at http://localhost:5173; HTTPS: API at https://localhost:5011/swagger, UI at https://localhost:5174
