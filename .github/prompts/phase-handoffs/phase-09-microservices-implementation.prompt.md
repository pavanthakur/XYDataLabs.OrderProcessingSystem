...
- Do not expose EF entities or domain entities from .PublicApi contracts.
...
Each .PublicApi project may contain only stable cross-module contracts:
- interfaces
- request DTOs
- response DTOs
- value objects intended for cross-module use

PublicApi projects must not reference Infrastructure projects.
PublicApi projects must not expose EF Core, DbContext, domain entities, provider SDK types, or controller types.
...
```csharp
namespace XYDataLabs.OrderProcessingSystem.Orders.PublicApi;
...
- Cross-module references go through .PublicApi projects.
...
2. PublicApi contracts for Orders, Payments, and Tenants.
